...
def approx_multiple_astar(G: nx.DiGraph, Trajets: list[tuple[str, str]], C: float) -> nx.DiGraph:
    def heuristic(u, v):
    ...
    G_reweighted = G.copy()
    N = len(Trajets)
    nx.set_edge_attributes(G_reweighted, False, "used")
    ...
    for trajet in generate_order(G, Trajets, mode="random"):
        ...
        for i in range(len(path) - 1):
            ...
            sum += G_reweighted[path[i]][path[i+1]]['distance']
