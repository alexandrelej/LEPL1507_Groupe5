...
from remove_edges import Remove_edges, precompute_shortest_paths
...
for n in n_values:
    ...
    for _ in range(num_runs):
        ...
        G_prime, edges_removed, best_cost = Remove_edges(random_subgraph, destination_pairs, C_values[0])
        ...
        start_time = time.time()
        G_reweighted, rsubgraph = Update_costs(random_subgraph, destination_pairs, C_values[0], iterations=30)
        elapsed_time = time.time() - start_time 
...
plt.subplot(1, 2, 2)
plt.plot(j_values, remove_edges_means_times_j, 'o-', label="Algorithme Remove edges")
plt.plot(j_values, update_costs_means_times_j, 's-', label="Algorithme Update costs")
plt.xlabel("Nombre de trajets")
plt.ylabel("Temps d'exécution (s)")
...
for C in C_values:
    ...
    print(f"Astar - Coût: {best_cost}, Temps: {remove_edges_time:.4f}s")
